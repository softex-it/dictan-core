...
public class XmlHtmlRemover {
    ...
    protected int inputProcessedLength;
    protected String output;

    private XmlHtmlRemover(String inInput, int inMaxLength) {
        this.input = inInput;
        this.maxLength = inMaxLength;
    }

    public static XmlHtmlRemover removeXmlHtml(String inInput, int inMaxLength) {
        XmlHtmlRemover instance = new XmlHtmlRemover(inInput, inMaxLength);
        instance.removeXmlHtml();
        return instance;
    }
    
    public static XmlHtmlRemover removeXmlHtml(String inInput) {
        return removeXmlHtml(inInput, -1);
    }

    protected void removeXmlHtml() {

        // Don't process blank input
        if (input == null) {
            return;
        } else if (StringUtils.isBlank(input)) {
            inputProcessedLength = input.length();
            output = input;
            return;
            //return new XmlProcessResult(input.length(), input, input);
        }
        ...
        for (; count < chars.length; count++) {
            ...
            if (curChar == '<') {
                ...
                if (htmlCharWithEnding != null) {
                    char[] htmlChar = Arrays.copyOfRange(htmlCharWithEnding, 0, htmlCharWithEnding.length - 1);
        ...
        inputProcessedLength = count;
        output = result.toString();
        //return new XmlProcessResult(count, input, result.toString());
    }
    ...
    protected static char[] readHtmlSpecialCharacter(char[] chars, int pointer) {
        ...
        char[] result = Arrays.copyOfRange(chars, pointer, j + 1);
    ...
    public String getProcessedInput() {
        return input.substring(0, getProcessedLength());
    }

    public boolean processedInputContains(CharSequence cs) {
//        char[] articlePartArr = new char[getProcessedLength()];
//        input.getChars(0, getProcessedLength(), articlePartArr, 0);
//        String articlePart = new String(articlePartArr).toLowerCase();
//        return articlePart.contains(cs);
        return getProcessedInput().contains(cs);
    }
